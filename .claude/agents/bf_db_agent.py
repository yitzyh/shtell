...
class BrowseForwardDB:
    ...
    def sample_urls(self, source: str, limit: int = 3) -> List[Dict[str, Any]]:
        """Get sample URLs from a source for quality assessment"""
        print(f"📋 Sampling {limit} URLs from source: {source}")
        print("=" * 60)
        ...
        # Sort by quality score or upvotes for better samples
        def get_quality_indicator(item):
            upvotes = int(item.get('upvotes', {}).get('N', 0))
            interactions = int(item.get('interactions', {}).get('N', 0))
            return upvotes + (interactions / 10)

        items.sort(key=get_quality_indicator, reverse=True)
        sampled = items[:limit]
        ...
        for i, item in enumerate(sampled, 1):
            ...
            print(f"\n{i}. {title}")
            print(f"   URL: {url}")
            print(f"   Upvotes: {upvotes} | Status: {status} | Quality: {quality}/100")
...
def main():
    ...
    sample_parser = subparsers.add_parser('sample-urls', help='Sample URLs from a source')
    sample_parser.add_argument('source', help='Source name (e.g., reddit-movies)')
    sample_parser.add_argument('--limit', type=int, default=3, help='Number of URLs to sample')
